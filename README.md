...
- Caja (RF18/CU12): `payment_provider=efectivo|tarjeta|datafono` (también `cash`/`card`) cobra **sin pasarela** con `InStorePaymentProvider` y guarda como referencia el comprobante interno `POS-<venta>`; funciona igual en producción, porque el pago lo confirma la tienda.
- Facturación: `FISCAL_PROVIDER=simulated` emite el documento de la venta con número determinista (`FAC-<venta>`), desglose de IVA (`FISCAL_TAX_RATE`, 19% por defecto, con el precio interpretado **con IVA incluido**) y datos de emisor configurables (`FISCAL_ISSUER_NAME`, `FISCAL_ISSUER_TAX_ID`); el campo `disclaimer` aclara que no tiene validez fiscal. Con `not_configured` (o cualquier otro valor) `POST /api/v1/commerce/sales/{id}/invoice` responde 503 a propósito: el recibo interno (`GET .../receipt`) no es una factura fiscal.
